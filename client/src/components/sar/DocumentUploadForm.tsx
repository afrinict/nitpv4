...
type DocumentField = keyof DocumentUploadForm;
...
const DocumentUploadForm: React.FC<DocumentUploadFormProps> = ({
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<DocumentUploadForm>({
    defaultValues: initialData,
  });

  const [uploadedFiles, setUploadedFiles] = useState<Partial<Record<DocumentField, File>>>({});
  const [uploadErrors, setUploadErrors] = useState<Partial<Record<DocumentField, string>>>({});

  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, fieldName: DocumentField) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-8">
          Please upload clear and legible copies of the following required documents. Max 5MB per file, accepted formats: PDF, JPEG, PNG.
        </p>
      </div>

      <div className="space-y-8">
        {/* Required Documents */}
        {renderFileUpload('titleDocument', 'Copy of Title Document / Certificate of Occupancy (C of O)', true)}
        {renderFileUpload('proofOfOwnership', 'Proof of Ownership (e.g., Allocation Letter, Survey Plan)', true)}
        {renderFileUpload('sitePlan', 'Site Plan (Proposed or Existing)', true)}

        {/* Corporate-specific Document */}
        {applicantType === 'corporate' && renderFileUpload('cacRegistration', 'CAC Registration Document', true)}

        {/* Additional Documents */}
        {renderFileUpload('additionalDocuments', 'Any Other Supporting Documents', false, true)}
      </div>
