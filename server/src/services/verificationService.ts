...
class VerificationService {
  ...
  private async getStoredOTP(key: string): Promise<string | null> {
  ...
  async sendEmailOTP(email: string, ip: string): Promise<{ success: boolean; message: string }> {
    try {
      // Check rate limit
  ...
  async sendWhatsAppMessage(phone: string, message: string, ip: string): Promise<{ success: boolean; message: string }> {
    try {
      // Check rate limit
  ...
  async verifyEmailOTP(email: string, otp: string, ip: string): Promise<{ success: boolean; message: string }> {
    try {
      const key = `email_otp:${email}`;
  ...
  async verifyPhoneOTP(phone: string, otp: string, ip: string): Promise<{ success: boolean; message: string }> {
    try {
      const key = `phone_otp:${phone}`;
      const storedOTP = await this.getStoredOTP(key);
      const attempts = await this.incrementAttempts(key);
