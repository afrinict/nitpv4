...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      const account = await financialService.reconcileBankAccount(
        req.params.id,
...
// Financial Reports
router.get('/reports/balance-sheet', async (req: any, res: any) => {
  try {
...
router.get('/reports/income-statement', async (req: any, res: any) => {
  try {
