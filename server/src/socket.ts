...
export function setupSocketServer(server: HTTPServer): Server {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('message:room', async (data: { roomId: number; message: string; senderId: number; attachmentUrl?: string }) => {
      try {
    ...
    socket.on('message:direct', async (data: { senderId: number; receiverId: number; message: string; attachmentUrl?: string }) => {
      try {
        // Store message in database
    ...
    socket.on('typing', (data: { userId: number; receiverId?: number; roomId?: number }) => {
      if (data.receiverId) {
        const receiverSocketId = userSockets.get(data.receiverId);
        if (receiverSocketId) {
    ...
    socket.on('election:vote', async (data: { electionId: number; positionId: number; candidateId: number }) => {
      try {
...
export function sendNotification(io: Server, userId: number, notification: any): void {
...
export function broadcastNotification(io: Server, notification: any): void {
