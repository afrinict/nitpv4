...
describe('GeolocationService', () => {
    ...
    describe('getLocationData', () => {
        it('should return cached data if available', async () => {
            redisClient.get.mockResolvedValue(JSON.stringify(mockLocationData));
        ...
        it('should fetch and cache new data if not cached', async () => {
            ...
            const result = await geolocationService.getLocationData(mockIP);
            expect(result).toEqual(mockLocationData);
            expect(redisClient.set).toHaveBeenCalledWith(`geo:${mockIP}`, JSON.stringify(mockLocationData), 'EX', 3600);
        });
    ...
    describe('blocked countries management', () => {
        it('should add a country to blocked list', async () => {
            ...
            expect(redisClient.set).toHaveBeenCalledWith('blocked_countries', JSON.stringify(['Test Country']));
        });
        it('should remove a country from blocked list', async () => {
            redisClient.get.mockResolvedValue(JSON.stringify(['Test Country']));
            await geolocationService.removeBlockedCountry('Test Country');
            expect(redisClient.set).toHaveBeenCalledWith('blocked_countries', JSON.stringify([]));
        });
    });
});
