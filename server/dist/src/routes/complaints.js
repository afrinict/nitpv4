...
router.get('/stats', async (req, res) => {
    try {
        const total = await db.select({ count: sql `count(*)` }).from(complaints);
        const received = await db.select({ count: sql `count(*)` }).from(complaints).where(eq(complaints.status, complaintStatusEnum.RECEIVED));
        const investigating = await db.select({ count: sql `count(*)` }).from(complaints).where(eq(complaints.status, complaintStatusEnum.INVESTIGATING));
        const resolved = await db.select({ count: sql `count(*)` }).from(complaints).where(eq(complaints.status, complaintStatusEnum.RESOLVED));
        const closed = await db.select({ count: sql `count(*)` }).from(complaints).where(eq(complaints.status, complaintStatusEnum.CLOSED));
        res.json({
            total: total[0].count,
