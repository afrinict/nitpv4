...
class FinancialService {
    ...
    async postJournalEntry(entryId, approvedBy) {
        try {
    ...
    // Bank Reconciliation
    async reconcileBankAccount(accountId, transactions) {
        try {
    ...
    // Tax Management
    async calculateTaxLiability(fiscalYear) {
        try {
            const revenueAccounts = await Account.find({ type: 'revenue', isActive: true });
            const expenseAccounts = await Account.find({ type: 'expense', isActive: true });
    ...
    async calculateCashFlowByType(type, startDate, endDate) {
    ...
    async calculateCurrentRatio(balanceSheet) {
        const currentAssets = Object.values(balanceSheet)
            .filter((account) => account.account.type === 'asset' && account.account.category === 'current')
            .reduce((sum, account) => sum + account.balance, 0);
        const currentLiabilities = Object.values(balanceSheet)
            .filter((account) => account.account.type === 'liability' && account.account.category === 'current')
            .reduce((sum, account) => sum + account.balance, 0);
        return currentLiabilities === 0 ? 0 : currentAssets / currentLiabilities;
    }
    async calculateDebtToEquityRatio(balanceSheet) {
        const totalLiabilities = Object.values(balanceSheet)
            .filter((account) => account.account.type === 'liability')
            .reduce((sum, account) => sum + account.balance, 0);
        const totalEquity = Object.values(balanceSheet)
            .filter((account) => account.account.type === 'equity')
            .reduce((sum, account) => sum + account.balance, 0);
        return totalEquity === 0 ? 0 : totalLiabilities / totalEquity;
    }
    async calculateReturnOnEquity(balanceSheet, date) {
    ...
    async calculateGrossProfitMargin(date) {
    ...
    async calculateTotalRevenue(date) {
        const revenueAccounts = await Account.find({ type: 'revenue', isActive: true });
        const startDate = new Date(date.getFullYear(), 0, 1);
        const revenue = await this.calculateAccountBalances(revenueAccounts, startDate, date);
        return revenue.total;
    }
    async calculateCostOfGoodsSold(date) {
    ...
    async analyzeBudgetVariance(fiscalYear, period) {
        try {
