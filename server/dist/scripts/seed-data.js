...
async function seedData() {
    ...
    try {
        ...
        for (const user of users) {
            ...
            await client.query(`INSERT INTO users (username, email, password, role, membership_id, is_verified)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (email) DO NOTHING
         RETURNING id`, [user.username, user.email, hashedPassword, user.role, user.membership_id, user.is_verified]);
        }
        // Insert members
        const members = [
            {
        ...
        for (const member of members) {
            await client.query(`INSERT INTO members (user_id, first_name, last_name, phone, address, membership_type, membership_status, membership_expiry)
         SELECT id, $1, $2, $3, $4, $5, $6, $7
         FROM users
         WHERE role = 'MEMBER'
         LIMIT 1
         ON CONFLICT DO NOTHING`, [member.first_name, member.last_name, member.phone, member.address, member.membership_type, member.membership_status, member.membership_expiry]);
        }
        // Insert events
        const events = [
            {
        ...
        for (const event of events) {
            await client.query(`INSERT INTO events (title, description, event_date, location, capacity, registration_fee, status)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT DO NOTHING`, [event.title, event.description, event.event_date, event.location, event.capacity, event.registration_fee, event.status]);
        }
        ...
        for (const transaction of transactions) {
            await client.query(`INSERT INTO financial_transactions (member_id, amount, type, status, description)
         SELECT m.id, $1, $2, $3, $4
         FROM members m
         LIMIT 1
         ON CONFLICT DO NOTHING`, [transaction.amount, transaction.type, transaction.status, transaction.description]);
        }
